...
handle_data :: proc "c" (e : ^webui.Event) {
	data_c := webui.webui_get_string(e)
	if data_c != nil {
		data_str := cast(string) data_c
...
main :: proc() {
	fmt.println("===========================================")
	fmt.println("  Odin + WebUI + Angular Integration")
	fmt.println("===========================================")
...
<head>
    ...
    <style>
        body { font-family: system-ui; background: linear-gradient(135deg, #1e3c72, #2a5298); 
        ...
        button { width: 100%; padding: 14px; background: linear-gradient(135deg, #1e3c72, #2a5298); 
                 color: white; border: none; border-radius: 8px; font-size: 16px; cursor: pointer; }
        .result { margin-top: 20px; padding: 15px; background: #f5f5f5; border-radius: 8px; text-align: center; }
    </style>
</head>
<body>
    <div class="container">
